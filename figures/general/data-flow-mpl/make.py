...
def plot_sub_package_icon(
    ax, offset=(0.5, 0.5), name=".makers", size=(22, 14), color="#3D3D3D", classes=[]
):
    p = Polygon(
...
def plot_image(ax):
    ...
    ax.set_title("Flux maps", color="#3D3D3D")
    format_dl5_ax(ax=ax)
...
def main(draft=True):
    ...
    plot_sub_package_icon(
        ax, offset=(37.5, 50), name=".makers", color="gray", classes=classes
    )
    ...
    plot_sub_package_icon(
        ax, offset=(105, 35), name=".estimators", color="gray", classes=classes
    )

    classes = ["Fit", "Models", "SkyModel", "FoVBackgroundModel", "etc."]
    plot_sub_package_icon(
        ax, offset=(105, 78), name=".modeling", color="gray", classes=classes
    )
    ...
    plot_curved_arrow(
        ax, posA=(91, 56), posB=(105, 42), connectionstyle=connectionstyle
    )
    ...
    xpos = 138
    plot_arrow(ax, offset=(xpos, 68), dx=6)
    plot_arrow(ax, offset=(xpos, 40), dx=6)
    plot_arrow(ax, offset=(xpos, 12), dx=6)
    plt.plot([138, 138], [12, 68], lw=3, color=color)
    plt.plot([128, 138], [40, 40], lw=3, color=color)
