"""
Plot example robust periodogram figures for the Gammapy docs at
http://docs.gammapy.org/dev/time/period.html
...
dt = 0.1  # define resolution of window function
time_win = np.rint(time / dt) * dt  # round time array of light curve to resolution
t_max = np.max(time_win)  # round maximum time of light curve
t_min = np.min(time_win)  # round minimum time of light curve
window_grid = np.arange(t_min, t_max + dt, dt)  # create evenly spaced time grid for window function
window_grid = np.rint(window_grid / dt) * dt   # round again since np.arange is not robust
window = np.zeros(len(window_grid))  # allocate container for window function

# set window function to one if a measurement was taken, zero otherwise
window[np.searchsorted(window_grid, time_win, side='right') - 1] = 1

# compute spectral window function
spectral_window_function = robust_periodogram(
    window_grid, window, np.ones_like(window),
...
fig = plot_periodogram(
    window_grid, window,
    periods=spectral_window_function['periods'],
    power=spectral_window_function['power']
)
